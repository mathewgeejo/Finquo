...
Finquo is a focused, responsive tool for mentors. Record audio in the browser or upload an existing session, review it, and use Gemini to turn the prominent topics into a downloadable word cloud.
...
- Genuine Gemini transcription and grounded prominent-term extraction
...
Prerequisites: Node.js 22.14 or newer and a Gemini API key.
...
Audio is converted to mono 16 kHz WAV before Gemini receives it. The uploaded Gemini file is deleted after each analysis request where the provider permits it. Provider retention and privacy remain subject to the Gemini API terms attached to the configured account.
...
Next.js, React, Google GenAI SDK, d3-cloud, Zod, Lucide React, FFmpeg/ffprobe static packages, Vitest, Playwright, TypeScript, and ESLint. No template or copied UI component was used.
